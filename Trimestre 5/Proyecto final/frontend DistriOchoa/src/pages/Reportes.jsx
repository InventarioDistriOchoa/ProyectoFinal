...
export default function Reportes() {
  ...
  const [resumen, setResumen] = useState({ productos: 0, categorias: 0, stockTotal: 0 });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setAlertasStock(stockData.map((p) => ({ producto: p.producto, estado: p.estado })));
          ...
          datasets: [
            {
              ...
              backgroundColor: ["#ffc107", "#198754", "#0d6efd", "#fd7e14", "#6f42c1"],
            },
          ],
        });

        // Entradas por mes
        const entradasRes = await fetch("http://localhost:3001/api/entrada/entrada", {
          headers: { Authorization: `Bearer ${token}` },
        });
        const entradasJson = await entradasRes.json();
        const entradas = Array.isArray(entradasJson.body) ? entradasJson.body : [entradasJson.body];
        const entradasPorMesData = {};
        entradas.forEach((e) => {
          const mes = new Date(e.Fecha).toLocaleString("es-ES", { month: "short" });
          entradasPorMesData[mes] = (entradasPorMesData[mes] || 0) + e.Cantidad;
        });
        setEntradasPorMes({
        ...
        const ventasRes = await fetch("http://localhost:3001/api/detalleVenta/detalleVenta", {
          headers: { Authorization: `Bearer ${token}` },
        });
        const ventasJson = await ventasRes.json();
        const ventas = Array.isArray(ventasJson.body) ? ventasJson.body : [ventasJson.body];
        const ventasCount = {};
        ventas.forEach((v) => {
          ventasCount[v.Producto_id] = (ventasCount[v.Producto_id] || 0) + v.Cantidad;
        });
  ...
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (sidebarRef.current && !sidebarRef.current.contains(e.target)) setSidebarVisible(false);
    };
  ...
  const renderIconoEstado = (estado) => {
    if (estado === "rojo") return <i className="bi bi-exclamation-triangle-fill text-danger me-2"></i>;
    if (estado === "amarillo") return <i className="bi bi-exclamation-circle-fill text-warning me-2"></i>;
    if (estado === "verde") return <i className="bi bi-check-circle-fill text-success me-2"></i>;
    return null;
  };
  ...
  return (
    <div className="min-vh-100 d-flex flex-column usuarios-background">
      {sidebarVisible && (
        <div className="position-fixed top-0 start-0 w-100 h-100" style={{ background: "rgba(0,0,0,0.3)", zIndex: 1900 }}></div>
      )}
      ...
      <header className="dashboard-header-top d-flex justify-content-between align-items-center px-4 py-2" style={{ backgroundColor: "transparent" }}>
        ...
        <div className="text-center flex-grow-1">
          <h2 className="fw-bold text-success m-0">📊 Reportes del Inventario</h2>
        </div>
        <div className="header-right d-flex align-items-center gap-3">
          <div className="dropdown">
            <button className="btn p-0 border-0 bg-transparent dropdown-toggle" type="button" id="perfilDropdown" data-bs-toggle="dropdown">
              <img src={`http://localhost:3001${foto}`} alt="Perfil" className="rounded-circle" style={{ width: "40px", height: "40px", objectFit: "cover", border: "2px solid #198754" }} />
            </button>
...
<div className="card p-3 shadow h-100">
  ...
  <ul className="list-group list-group-flush">
    {alertasStock.length > 0 ? (
      alertasStock
        .filter(a => a.producto.toLowerCase().includes(busqueda.toLowerCase()))
      ...
      <li className="list-group-item text-success d-flex align-items-center">
        <i className="bi bi-check-circle-fill me-2"></i> Todo el stock está en óptimas condiciones
      </li>
    )}
  </ul>

  {/* Paginación */}
  {alertasStock.filter(a => a.producto.toLowerCase().includes(busqueda.toLowerCase())).length > ITEMS_POR_PAGINA && (
    <div className="d-flex justify-content-between mt-2">
      ...
      <span className="align-self-center">
        {paginaActual} / {Math.ceil(alertasStock.filter(a => a.producto.toLowerCase().includes(busqueda.toLowerCase())).length / ITEMS_POR_PAGINA)}
      </span>
      <button
        className="btn btn-sm btn-outline-secondary"
        disabled={paginaActual === Math.ceil(alertasStock.filter(a => a.producto.toLowerCase().includes(busqueda.toLowerCase())).length / ITEMS_POR_PAGINA)}
        onClick={() => setPaginaActual(prev => prev + 1)}
      >
        Siguiente
      </button>
            ...
            <div className="card p-3 shadow h-100">
              ...
              <ul className="list-group list-group-flush small">
                <li className="list-group-item"><i className="bi bi-box-seam text-success me-2"></i> Productos registrados: <strong>{resumen.productos}</strong></li>
                <li className="list-group-item"><i className="bi bi-tags text-info me-2"></i> Categorías activas: <strong>{resumen.categorias}</strong></li>
                <li className="list-group-item"><i className="bi bi-archive-fill text-warning me-2"></i> Stock total disponible: <strong>{resumen.stockTotal}</strong></li>
              </ul>
            </div>
          </div>
            ...
            <div className="card p-3 shadow" style={{ height: "auto" }}>
              ...
              {stockActual && (
                <div style={{ height: "400px" }}>
                  <Bar data={stockActual} options={{ indexAxis: "y", maintainAspectRatio: false }} />
                </div>
              )}
            </div>
          </div>
