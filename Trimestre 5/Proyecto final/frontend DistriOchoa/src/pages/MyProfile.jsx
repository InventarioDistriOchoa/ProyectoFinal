...
export default function MyProfile() {
    ...
    const fetchData = async () => {
      try {
        const [profileRes, tiposRes] = await Promise.all([
          fetch("http://localhost:3001/api/persona/me", {
            headers: { Authorization: `Bearer ${token}` },
          }),
          fetch("http://localhost:3001/api/tipoDocumento/tipoDocumento", {
            headers: { Authorization: `Bearer ${token}` },
          }),
        ]);
        if (!profileRes.ok || !tiposRes.ok)
  ...
  const tipoDocDescripcion =
    tiposDocumento.find(
      (td) => td.idTipo_Documento === profile?.Tipo_Documento_id
    )?.Descripcion || profile?.Tipo_Documento_id;
  ...
  const handleFotoChange = async (file) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/persona/persona/${profile.idPersona}/foto`,
        {
          method: "PUT",
          headers: { Authorization: `Bearer ${token}` },
          body: formData,
      ...
      if (res.ok) {
        ...
        const profileRes = await fetch("http://localhost:3001/api/persona/me", {
          headers: { Authorization: `Bearer ${token}` },
        });
        const profileData = await profileRes.json();
        setProfile(profileData.body);
        ...
        Swal.fire(
          "Error",
          data.Message || data.message || "No se pudo subir la foto",
          "error"
        );
      }
    } catch (err) {
  ...
  const handleEditar = async (e) => {
    ...
    const payload = {
      ...
      ...(profile.Contrasena?.trim()
        ? { Contrasena: profile.Contrasena.trim() }
        : {}),
    };

    try {
      const res = await fetch(
        `http://localhost:3001/api/persona/persona/${profile.idPersona}`,
        {
          method: "PUT",
          headers: {
      ...
      if (res.ok) {
        ...
        const profileRes = await fetch(
          "http://localhost:3001/api/persona/me",
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const profileData = await profileRes.json();
        setProfile(profileData.body);
      } else {
        console.log("❌ Error al actualizar perfil:", data);
        Swal.fire(
          "Error",
            ...
            <div className="card shadow-lg rounded-4 p-4">
              ...
              <div className="d-flex flex-column align-items-center text-center position-relative">
                <img
                  src={
                    profile.Foto
                      ? `http://localhost:3001${profile.Foto}`
                ...
                <p className="text-muted mb-3">
                  {profile.Rol?.Descripcion_Rol || "Empleado"}
                </p>
              </div>
              ...
              <div className="row mt-4">
                <div className="col-md-6 mb-3">
                  <div className="card shadow-sm rounded-4 p-3 text-center ">
                    ...
                    <p className="mb-0" style={{ marginTop: "13px" }}>
                      {profile.Nombre}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="modal-body">
                    ...
                    <div className="mb-3">
                      ...
                      <select
                        ...
                        value={profile.Tipo_Documento_id}
                        onChange={(e) =>
                          setProfile((prev) => ({
                            ...prev,
                      ...
                      >
                        ...
                        {tiposDocumento.map((td) => (
                          <option
                            key={td.idTipo_Documento}
                            value={td.idTipo_Documento}
                          >
                            {td.Descripcion}
                          </option>
                        ))}
