...
export default function GestionTipoDoc() {
...
const sidebarItems = [
  ...
  // Solo mostrar módulo de usuarios si el rol es admin
  ...(rolUsuario === "admin" ? [
    { label: "Usuarios", icon: <BiUser />, action: () => navigate("/usuarios") },
    { label: "Roles", icon: <BiFile />, action: () => navigate("/roles") },
    { label: "Registro Usuarios", icon: <BiFile />, action: () => navigate("/registro-usuarios") },
  ...
  // Iconos según tipo
  const getIconoTipo = (descripcion) => {
    switch (descripcion.toLowerCase()) {
      case "cedula de ciudadania":
        return "/img/icon-cedula.jpg";
      case "tarjeta de identidad": // <-- aquí
        return "/img/icon-tarjeta.png";
      case "cedula extranjera":
        return "/img/icon-cedula-ext.png";
      case "pasaporte":
        return "/img/icon-pasaporte.png";
      default:
        return "/img/icon-default2.png";
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (descripcion.trim().length < 3) return Swal.fire("Error", "La descripción debe tener al menos 3 caracteres", "warning");
    ...
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        Swal.fire("Éxito", "Tipo de documento creado", "success");
        setTiposDoc(prev => [...prev, { idTipo_Documento: data.body.idTipo_Documento, Descripcion: descripcion }]);
        setDescripcion("");
  ...
  const handleEliminar = async (id) => {
    const confirm = await Swal.fire({ title: "¿Eliminar este tipo de documento?", icon: "warning", showCancelButton: true, confirmButtonText: "Sí, eliminar", cancelButtonText: "Cancelar" });
    if (confirm.isConfirmed) {
      try {
        const res = await fetch(`http://localhost:3001/api/tipoDocumento/tipoDocumento/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
        const data = await res.json();
        if (res.ok) {
          setTiposDoc(prev => prev.filter(t => t.idTipo_Documento !== id));
          Swal.fire("Eliminado", "Tipo de documento eliminado correctamente", "success");
  ...
  const abrirModalEditar = (tipo) => setEditDoc({ ...tipo });
    ...
    >
      ...
      <header className="dashboard-header-top d-flex align-items-center px-4 py-2 position-relative" style={{ backgroundColor: "transparent" }}>
        ...
        <div className="header-center position-absolute top-50 start-50 translate-middle text-center">
          <div className="app-name" style={{ color: "#198754", fontWeight: "bold", fontSize: "1.9rem" }}>Gestión de Tipos de Documento</div>
        </div>
      ...
      <div className="container mt-4 px-3 pt-5">
        ...
        {vista === "registro" && (
          <div className="d-flex justify-content-center">
            <div className="card shadow-lg p-4 rounded-5" style={{ maxWidth: "500px", width: "100%" }}>
              <h4 className="text-center mb-4" style={{ color: "#198754" }}>Registro de Tipo de Documento</h4>
              <form onSubmit={handleSubmit}>
                ...
                <div className="d-flex justify-content-center">
                  <button type="submit" className="btn btn-success btn-lg px-5">Registrar</button>
                </div>
              </form>
            </div>
          </div>
        ...
        {vista === "listado" && (
          <div className="row">
            {tiposDoc.length === 0 ? (
              <p className="text-center text-muted">No hay tipos de documento registrados.</p>
            ) : (
              tiposDoc.map((t, index) => (
                <div className="col-md-6 col-lg-4 mb-3" key={index}>
                  <div className="card shadow-sm h-100 text-center">
                    <div className="card-body d-flex flex-column align-items-center">
                      <img src={getIconoTipo(t.Descripcion)} alt={t.Descripcion} style={{ width: "60px", height: "60px", marginBottom: "10px" }} />
                      <h6 className="card-title mb-2">{t.Descripcion}</h6>
                      <div className="d-flex justify-content-center gap-3 mt-auto">
                        <span className="text-primary" style={{ cursor: "pointer", fontSize: "1.2rem" }} title="Editar" onClick={() => abrirModalEditar(t)}><BiPencil /></span>
                        <span className="text-danger" style={{ cursor: "pointer", fontSize: "1.2rem" }} title="Eliminar" onClick={() => handleEliminar(t.idTipo_Documento)}><BiTrash /></span>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content">
                <form onSubmit={async e => {
                  e.preventDefault();
                  try {
                    const res = await fetch(`http://localhost:3001/api/tipoDocumento/tipoDocumento/${editDoc.idTipo_Documento}`, {
                      method: "PUT",
                      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                      body: JSON.stringify({ Descripcion: editDoc.Descripcion }),
                    });
                    const data = await res.json();
                    if (res.ok) {
                      Swal.fire("Éxito", "Tipo de documento actualizado", "success");
                      setTiposDoc(prev => prev.map(t => t.idTipo_Documento === editDoc.idTipo_Documento ? editDoc : t));
                      setEditDoc(null);
                  ...
                  <div className="modal-header">
                    ...
                    <button type="button" className="btn-close" onClick={() => setEditDoc(null)}></button>
                  </div>
                  <div className="modal-body">
                    <input type="text" className="form-control" value={editDoc.Descripcion} onChange={e => setEditDoc(prev => ({ ...prev, Descripcion: e.target.value }))} required />
                  </div>
                  <div className="modal-footer">
                    <button type="button" className="btn btn-secondary" onClick={() => setEditDoc(null)}>Cancelar</button>
