...
import { BiHome, BiBox, BiUser, BiCategory, BiLogOut, BiFile, BiPencil, BiTrash } from "react-icons/bi";
...
export default function GestionProveedores() {
  ...
  const mostrarModuloUsuarios = true; // Ajusta según tus permisos o lógica de rol
  const sidebarItems = [
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (nombreEmpresa.trim().length < 3) return Swal.fire("Error", "El nombre debe tener al menos 3 caracteres", "warning");

    const nuevoProveedor = { Nombre_Empresa: nombreEmpresa, Direccion: direccion };
    try {
      const res = await fetch("http://localhost:3001/api/proveedor/proveedor", {
        method: "POST",
      ...
      if (res.ok) {
        ...
        setProveedores(prev => [...prev, { ...nuevoProveedor, idProveedor: data.body.idProveedor }]);
        setNombreEmpresa("");
        setDireccion("");
  ...
  const handleEliminar = async (id) => {
    const confirm = await Swal.fire({ title: "¿Eliminar este proveedor?", icon: "warning", showCancelButton: true, confirmButtonText: "Sí, eliminar", cancelButtonText: "Cancelar" });
    if (confirm.isConfirmed) {
      try {
        const res = await fetch(`http://localhost:3001/api/proveedor/proveedor/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
        const data = await res.json();
        if (res.ok) {
    ...
    >
      ...
      <header className="dashboard-header-top d-flex align-items-center px-4 py-2 position-relative" style={{ backgroundColor: "transparent" }}>
        ...
        <div className="header-center position-absolute top-50 start-50 translate-middle text-center">
          <div className="app-name" style={{ color: "#198754", fontWeight: "bold", fontSize: "1.9rem" }}>Gestión de Proveedores</div>
        </div>
        ...
        {vista === "registro" && (
          <div className="d-flex justify-content-center">
            <div className="card shadow-lg p-4 rounded-5" style={{ maxWidth: "700px", width: "100%" }}>
              <h4 className="text-center mb-4" style={{ color: "#198754" }}>Registro de Proveedor</h4>
              <form onSubmit={handleSubmit} className="row g-3">
                <div className="col-md-6">
                  <label className="form-label">Nombre Empresa</label>
                  <input type="text" className="form-control rounded-3 shadow-sm" value={nombreEmpresa} onChange={e => setNombreEmpresa(e.target.value)} required />
                </div>
                <div className="col-md-6">
                  <label className="form-label">Dirección</label>
                  <input type="text" className="form-control rounded-3 shadow-sm" value={direccion} onChange={e => setDireccion(e.target.value)} />
                </div>
        ...
        {vista === "listado" && (
          <div className="row">
            {proveedores.length === 0 ? (
              <p className="text-center text-muted">No hay proveedores registrados.</p>
            ) : (
                  ...
                  <div className="card shadow-sm h-100 text-center">
                    <div className="card-body">
                      ...
                      <div className="d-flex justify-content-center gap-3">
                        <span className="text-primary" style={{ cursor: "pointer", fontSize: "1.2rem" }} title="Editar" onClick={() => abrirModalEditar(p)}><BiPencil /></span>
                        <span className="text-danger" style={{ cursor: "pointer", fontSize: "1.2rem" }} title="Eliminar" onClick={() => handleEliminar(p.idProveedor)}><BiTrash /></span>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div className="modal-content">
                <form
                  onSubmit={async (e) => {
                    e.preventDefault();
                    try {
                      const res = await fetch(`http://localhost:3001/api/proveedor/proveedor/${editProveedor.idProveedor}`, {
                        method: "PUT",
                        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
                        body: JSON.stringify({ Nombre_Empresa: editProveedor.Nombre_Empresa, Direccion: editProveedor.Direccion }),
                      });
                      const data = await res.json();
                      if (res.ok) {
                        Swal.fire("Éxito", "Proveedor actualizado", "success");
                        setProveedores(prev => prev.map(p => p.idProveedor === editProveedor.idProveedor ? editProveedor : p));
                ...
                >
                  <div className="modal-header">
                    ...
                    <button type="button" className="btn-close" onClick={() => setEditProveedor(null)}></button>
                  </div>
                  <div className="modal-body">
                    <div className="mb-3">
                      <label className="form-label">Nombre Empresa</label>
                      <input type="text" className="form-control" value={editProveedor.Nombre_Empresa} onChange={e => setEditProveedor(prev => ({ ...prev, Nombre_Empresa: e.target.value }))} required />
                    </div>
                    <div className="mb-3">
                      <label className="form-label">Dirección</label>
                      <input type="text" className="form-control" value={editProveedor.Direccion} onChange={e => setEditProveedor(prev => ({ ...prev, Direccion: e.target.value }))} />
                    </div>
                  </div>
                  <div className="modal-footer">
                    <button type="button" className="btn btn-secondary" onClick={() => setEditProveedor(null)}>Cancelar</button>
