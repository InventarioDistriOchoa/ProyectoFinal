...
export default function GestionCategorias() {
  ...
  // ---------- Registrar categoría (con reactivar si está desactivada) ----------
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(
        "http://localhost:3001/api/categoria/categoria",
        {
          method: "POST",
          headers: {
      ...
      // 👉 CASO ESPECIAL: la categoría ya existe pero está desactivada
      if (res.status === 409 && data.desactivado) {
        const result = await Swal.fire({
          icon: "warning",
          title: "La categoría ya existe",
          text: data.Message || "Esta categoría está desactivada. ¿Deseas activarla?",
        ...
        if (result.isConfirmed) {
          // Llamada para ACTIVAR la categoría
          const activarRes = await fetch(
            `http://localhost:3001/api/categoria/categoria/activar/${data.idCategoria}`,
            {
              method: "PUT",
              headers: {
                Authorization: `Bearer ${token}`,
          ...
          if (activarRes.ok) {
            ...
            Swal.fire(
              "Error",
              activarData.Message || "No se pudo activar la categoría",
              "error"
            );
          }
        }
      ...
      // 👉 CASO NORMAL: creada correctamente
      if (res.ok) {
  ...
  // ---------- Eliminar (soft delete en el back) ----------
  const handleEliminar = async (id) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/categoria/categoria/${id}`,
        {
          method: "DELETE",
          headers: { Authorization: `Bearer ${token}` },
        }
