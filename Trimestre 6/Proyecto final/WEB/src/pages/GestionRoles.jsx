...
export default function GestionRoles() {
  ...
  const getIconoRol = (descripcionRol) => {
    switch (descripcionRol.toLowerCase()) {
      case "admin":
        return "/img/icon-admin.png";
      case "superadmin":
        return "/img/icon-superadmin.png";
      case "auxiliar":
        return "/img/icon-usuario.png";
      default:
  ...
  // ---------- Crear Rol (con lógica de reactivar) ----------
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (descripcionRol.trim().length < 3)
      return Swal.fire(
        "Error",
    ...
    const nuevoRol = { Descripcion_Rol: descripcionRol.trim() };
    try {
      const res = await fetch("http://localhost:3001/api/rol/rol", {
        method: "POST",
        headers: {
      ...
      if (res.status === 409 && data.desactivado && data.idRol) {
        ...
        if (result.isConfirmed) {
          // Llamada para ACTIVAR el rol
          const activarRes = await fetch(
            `http://localhost:3001/api/rol/rol/activar/${data.idRol}`,
            {
              method: "PUT",
              headers: { Authorization: `Bearer ${token}` },
            }
          ...
          if (activarRes.ok) {
            ...
            Swal.fire(
              "Error",
              activarData.Message || "No se pudo activar el rol",
              "error"
            );
          }
        }
      ...
      // 👉 Otros errores
      Swal.fire("Error", data.Message || data.message || "No se pudo crear", "error");
    } catch (err) {
      console.error(err);
    ...
    if (confirm.isConfirmed) {
      try {
        const res = await fetch(
          `http://localhost:3001/api/rol/rol/${id}`,
          {
            method: "DELETE",
            headers: { Authorization: `Bearer ${token}` },
          }
        ...
        if (res.ok) {
          ...
          Swal.fire("Eliminado", "Rol desactivado correctamente", "success");
        } else
          Swal.fire(
            "No se pudo eliminar",
              ...
              <div className="modal-content">
                <form
                  onSubmit={async (e) => {
                    ...
                    try {
                      const res = await fetch(
                        `http://localhost:3001/api/rol/rol/${editRol.idRol}`,
                        {
                          method: "PUT",
                          headers: {
                            "Content-Type": "application/json",
                          ...
                          body: JSON.stringify({
                            Descripcion_Rol: editRol.Descripcion_Rol,
                          }),
                        }
                      );
                      const data = await res.json();
                      if (res.ok) {
                        ...
                        setRoles((prev) =>
                          prev.map((r) =>
                            r.idRol === editRol.idRol ? editRol : r
                          )
                        );
                        setEditRol(null);
                      } else
                  ...
                  <div className="modal-body">
                    <div className="mb-3">
                      ...
                      <input
                        ...
                        value={editRol.Descripcion_Rol}
                        onChange={(e) =>
                          setEditRol((prev) => ({
                            ...prev,
                            Descripcion_Rol: e.target.value,
                          }))
                        }
                        required
                      />
