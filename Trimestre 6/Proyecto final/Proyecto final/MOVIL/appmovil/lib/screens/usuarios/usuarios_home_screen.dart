...
class UsuariosHomeScreen extends StatefulWidget {
  const UsuariosHomeScreen({super.key});

  @override
  State<UsuariosHomeScreen> createState() => _UsuariosHomeScreenState();
}

class _UsuariosHomeScreenState extends State<UsuariosHomeScreen> {
  ...
  String get _rolLabel {
    final r = _rol.toLowerCase();
    if (r == 'admin') return 'Eres Admin 🛠️';
    if (r == 'superadmin') return 'Eres Superadmin 👑';
    if (r.isEmpty) return 'Rol: ...';
    return 'Rol: $r';
  }
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (foto != null && foto.toString().trim().isNotEmpty) {
          ...
          setState(() => _fotoUrl = f.startsWith('http') ? f : '${EnvConfig.mediaBaseUrl}$f');
        }
      }
    } catch (_) {
      // silencio
  ...
  List<_SideItem> _itemsSidebar() {
    return [
      ...
      if (_mostrarModuloUsuarios) _SideItem('Usuarios', Icons.people_alt_outlined, () => Navigator.pushReplacementNamed(context, '/usuarios')),
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
                    ...
                    children: [
                      ...
                      PopupMenuButton<String>(
                        ...
                        onSelected: (v) {
                          ...
                          if (v == 'inicio') Navigator.pushReplacementNamed(context, '/dashboard');
          ...
          // ✅ CONTENIDO CON SCROLL (lo que te faltaba)
          Positioned.fill(
            top: 92,
            bottom: 18,
            child: LayoutBuilder(
                            ...
                            Padding(
                              ...
                              child: Center(
                                child: Wrap(
                                  ...
                                  children: [
                                    _cardOpcion(
                                      img: 'assets/img/roles-icono.png',
                                      text: 'Roles',
                                      subtitle: 'Crear + editar + eliminar',
                                      onTap: () => Navigator.pushNamed(context, '/gestion-roles'),
                                    ),
                                    _cardOpcion(
                                      img: 'assets/img/icon-proveedores.png',
                                      text: 'Proveedores',
                                      subtitle: 'Crear + listado + editar',
                                      onTap: () => Navigator.pushNamed(context, '/proveedores'),
                                    ),
                                    _cardOpcion(
                                      img: 'assets/img/icon-usuarios.png',
                                      text: 'Crear Usuarios',
                                      subtitle: 'Registro + listado + activar',
                                      onTap: () => Navigator.pushNamed(context, '/usuarios'),
                                    ),
                                    _cardOpcion(
                                      img: 'assets/img/icon-documentos.png',
                                      text: 'Tipo Documento',
                                      subtitle: 'Crear + editar + eliminar',
                                      onTap: () => Navigator.pushNamed(context, '/tipo-documento'),
                                    ),

                                    // ✅ AGREGA MÁS AQUÍ y seguirá scrolleando sin problema
                                    // _cardOpcion(...),
                                  ],
                                ),
                              ),
